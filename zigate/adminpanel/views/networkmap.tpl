...
<div id="zigatenetworkmap" style="height:700px"></div>
...
$(document).ready(function () {
    ...
    window.networkGraph = new vis.Network(container, data, _networkOptions);
    refreshData();
});
...
function refreshData(force = false) {
    ...
    $.getJSON('{{get_url('api_devices')}}', function (devices) {
        ...
        $.getJSON('{{get_url('api_network_table')}}', { 'force': force }, function (data) {
			...
			window.networkGraph.stabilize();
			window.networkGraph.physics.options.enabled = false
        });
    });
